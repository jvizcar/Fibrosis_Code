...
def count_image(gc, item_id, params, metadata_key=None, roi_doc_name=None, roi_group_name=None, save_dir=None,
                tile_dim=5000, color_map=None, file_path=None):
    ...
    roi_doc_name : str (optional)
        if roi_group_name provided, you can specify which annotation document to get annotation elements from, if None
        then all annotationd documents will be searched. Note that all documents that start with this string will be
        included.
    roi_group_name : str (optional)
        specify which element group to get the roi from
    ...
    if save_dir is not None:
        # create saving directory if not created
    ...
    # handle getting the roi binary mask
    roi_contours = get_annotation_mask(gc, item_id, roi_group_name=roi_group_name, roi_doc_name=roi_doc_name,
                                       scale_factor=scale_factor)

    # create empty roi mask
    roi_mask = np.zeros((int(height * scale_factor), int(width * scale_factor)),
                        dtype=np.uint8)

    if len(roi_contours):
        # if there are roi contours, draw them in the roi mask and fill inside
        roi_mask = cv2.drawContours(roi_mask, roi_contours, -1, color=255, thickness=cv2.FILLED)
    else:
        # else the whole mask is the roi
        roi_mask[:, :] = 255
    ...
    for y, tile_top in enumerate(range(top, bottom, tile_dim)):
